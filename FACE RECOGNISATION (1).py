...
def startface(frame,cv2image,lmain):
    clf=cv2.CascadeClassifier("D:\haarcascade_frontalface_default.xml")
...
def start():
    ...
    btn_DetectFace=Button(imageFrame,command=lambda:startface(),text='recognize face',font=('',20),bd=5,width=12).place(relx=.1,rely=.85)
...
def train_model():
    global model,pca
    clf=cv2.CascadeClassifier('D:\haarcascade_frontalface_default.xml')
    classes=os.listdir('D:/images')
    Y=[]
    X=[]
    for cls in classes:
        for img in os.listdir("D:/images/"+cls):
